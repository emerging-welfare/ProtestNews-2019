...
df["offset"] = ""
df["text"] = ""
listed_news = os.listdir("../collector/tmp/texts")
print(len(listed_news))
...
for url in df.url.unique().tolist():
    ...
    #print(text_file)
    text_file_full = "../collector/tmp/texts/" + text_file
    if text_file not in listed_news:
        continue
    else:
        with open(text_file_full, "r", encoding="utf-8") as f:
            text = f.read()
